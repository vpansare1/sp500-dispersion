...
def clean_extreme_returns(returns_cs: pd.Series) -> pd.Series:
    """Replace inf/-inf with NaN in a cross-section of returns.

    Note: this no longer caps large returns. An earlier version dropped returns
    above +/-400% as presumed glitches, but that discarded genuine extreme
    moves (e.g. semiconductor names up 500-750% in the 2026 memory supercycle),
    distorting dispersion downward. Protection against bad data now lives in the
    download layer (retry + coverage + cross-section guards in download_prices /
    daily_update), which targets the real failure mode -- thin/failed Yahoo
    pulls -- without touching legitimate returns.
    """
    return returns_cs.replace([np.inf, -np.inf], np.nan)
...
def horizon_returns(prices: pd.DataFrame, window: int) -> pd.DataFrame:
    ...
    return (prices / prices.shift(window) - 1.0).replace([np.inf, -np.inf], np.nan)
...
def compute_equal_weighted_metrics(prices: pd.DataFrame,
    ...
    """
    daily_rets = prices.pct_change(fill_method=None).replace([np.inf, -np.inf], np.nan)
